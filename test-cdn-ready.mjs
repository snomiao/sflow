...
const checks = {
  ...
  'exports["."].import points to dist': pkg.exports?.['.']?.import === './dist/index.js',
};
...
const distIndexContent = readFileSync(join(__dirname, 'dist/index.js'), 'utf-8');
const hasNodeImports = /from\s+["']node:/.test(distIndexContent);
console.log(`   ${!hasNodeImports ? '✓' : '✗'} No Node.js imports in dist/index.js`);

// Test 3: Check UMD build exists
console.log('\n3️⃣  Checking UMD build...');
try {
  const umdContent = readFileSync(join(__dirname, 'dist/index.umd.js'), 'utf-8');
  const hasUMDWrapper = /typeof exports === 'object' && typeof module !== 'undefined'/.test(umdContent);
  console.log(`   ✓ UMD build exists (${(umdContent.length / 1024).toFixed(2)} KB)`);
